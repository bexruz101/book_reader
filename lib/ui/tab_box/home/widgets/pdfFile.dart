...
class PdfScreen extends StatefulWidget {
  ...
  PdfScreen(
      {super.key, required this.bookPath,required this.bookPages, required this.extractor, required this.document});
}

class _PdfScreenState extends State<PdfScreen> {
  late PerfectPdfController _controller;
  int page = 0;
  int currentPage = StorageRepository.getInt('currPage');
  int handleMargin = 50;
  ...
  @override
  void initState() {
    currentPage = StorageRepository.getInt('currPage');
    super.initState();
  }
        ...
        body: GestureDetector(
          ...
          child: Stack(
              children: [PerfectPdfRenderer(
                ...
                pages: widget.bookPages,
                onMaxWidthSet: (int maxWidthInt) {
                  this.initZoom = screenWidth /
                      widget.document.pages[maxWidthInt].size.width;
                },
                extractionEnd: () {
                  isExtraction = false;
                },
                ...
                isExtraction ?
                Positioned(
                    left: isExchanged ? x1 - handleMargin / 2 : x1 -
                        handleWidth - handleMargin / 2,
                ...
                isExtraction ?
                Positioned(
                  left: isExchanged ? x2 - handleWidth - handleMargin / 2 : x2 -
                      handleMargin / 2,
  ...
  bool setInitHandlePosition(double x, double y) {
    ...
    for (int i = 0; i < offsets.length - 2; i ++) {
      if (i == offsets.length - 3 || y > offsets[i][0] && y <= offsets[i+1][0]) {
        ...
        pageInd = i;
        cache[page] = widget.extractor.extractTextLines(
    ...
    for (int j = 0; j < textLines.length; j ++) {
      ...
      right = min(textLine.bounds.right * offsets[0][3] * initZoom +
          offsets[pageInd][1], screenWidth);
      left = max(
          textLine.bounds.left * offsets[0][3] * initZoom + offsets[pageInd][1],
          0);
      top = max(
          textLine.bounds.top * offsets[0][3] * initZoom + offsets[pageInd][0],
          0);
      bottom = min(textLine.bounds.bottom * offsets[0][3] * initZoom +
          offsets[pageInd][0], screenHeight);
    ...
    for (int j = 0; j < words.length; j++) {
      word = words[j];
      right = min(
          word.bounds.right * offsets[0][3] * initZoom + offsets[pageInd][1],
          screenWidth);
      left = max(
          word.bounds.left * offsets[0][3] * initZoom + offsets[pageInd][1], 0);
      top = max(
          word.bounds.top * offsets[0][3] * initZoom + offsets[pageInd][0], 0);
      bottom = min(
          word.bounds.bottom * offsets[0][3] * initZoom + offsets[pageInd][0],
          screenHeight);
  ...
  void recalculate(int handle) {
    ...
    if (handle == 1) {
      ...
      prevY = (y1 - prevYOffset) / (initZoom * offsets[0][3]) -
          (cache[prevPage]![prevLine].bounds.height) / 2;
      prevX1 = x1;
      prevY1 = y1;
    }
    else {
      ...
      prevY = (y2 - prevYOffset) / (initZoom * offsets[0][3]) -
          (cache[prevPage]![prevLine].bounds.height) / 2;
      prevX1 = x2;
      prevY1 = y2;
    }

    double draggingX = (this.draggingX - prevXOffset) /
        (initZoom * offsets[0][3]);
    double draggingY = (this.draggingY - prevYOffset) /
        (initZoom * offsets[0][3]) -
        (cache[prevPage]![prevLine].bounds.height) / 2;
    ...
    while (left >= 0 || right < textLines.length) {
      ...
      if ((textLines[current].bounds.top +
          (cache[prevPage]![current].bounds.height) / 2 - draggingY).abs() <=
          diffY && doLinesIntersectY(textLines[prevLine], textLines[current])) {
        isLineFound = true;
        if (handle == 1) {
          y1 = textLines[current].bounds.bottom * initZoom * offsets[0][3] +
              prevYOffset;
        }
        else {
          y2 = textLines[current].bounds.bottom * initZoom * offsets[0][3] +
              prevYOffset;
        }
        if ((handle == 1 && !isExchanged) || (handle == 2 && isExchanged)) {
          prevLine = current;
    ...
    if (!isLineFound && textLines[prevLine].bounds.left > draggingX) {
      for (int i = 0; i < textLines.length; i++) {
        if (textLines[prevLine].bounds.left - draggingX >
            (textLines[i].bounds.right - draggingX).abs() &&
            doLinesIntersectX(textLines[prevLine], textLines[i])) {
          prevLine = i;
        }
      }
    }
      ...
      for (int i = 0; i < textLines.length; i++) {
        if (draggingX - textLines[prevLine].bounds.left >
            (textLines[i].bounds.left - draggingX).abs() &&
            doLinesIntersectX(textLines[prevLine], textLines[i])) {
          prevLine = i;
        }
      }
    ...
    if (handle == 1) {
      y1 = textLines[prevLine].bounds.bottom * initZoom * offsets[0][3] +
          prevYOffset;
    }
    else {
      y2 = textLines[prevLine].bounds.bottom * initZoom * offsets[0][3] +
          prevYOffset;
    }
    ...
    for (int i = 0; i < words.length; i++) {
      if (words[i].bounds.left < draggingX &&
          draggingX <= words[i].bounds.right) {
        isWordFound = true;
        if ((handle == 1 && !isExchanged) || (handle == 2 && isExchanged)) {
          prevWord = i;
          fromWord1 = i;
      ...
      current = min((draggingX - words[i].bounds.left).abs(),
          (draggingX - words[i].bounds.right).abs());
      if (i == 0) {
        differ = current;
        prevWord = i;
      }
    ...
    if (isWordFound) {
      ...
      for (int i = 0; i < glyphs.length; i++) {
        prevChar = i;
        if (glyphs[i].bounds.left < draggingX &&
            draggingX <= glyphs[i].bounds.right) {
          break;
        }
      }
      ...
      if (prevX > draggingX) {
        if (handle == 1) {
          x1 = glyphs[prevChar].bounds.right * initZoom * offsets[0][3] +
              prevXOffset;
        }
        else {
          x2 = glyphs[prevChar].bounds.right * initZoom * offsets[0][3] +
              prevXOffset;
        }
      ...
      if (prevX < draggingX) {
        ...
        if (handle == 1) {
          x1 = glyphs[prevChar].bounds.left * initZoom * offsets[0][3] +
              prevXOffset;
        }
        else {
          x2 = glyphs[prevChar].bounds.left * initZoom * offsets[0][3] +
              prevXOffset;
        }
      }
    }
    ...
    if (fromLine2 < fromLine1 || (fromLine1 == fromLine2 && fromWord2 < fromWord1) || (
        fromLine1 == fromLine2 && fromWord1 == fromWord2 &&
            fromChar2 <= fromChar1)) {
      v = fromChar1;
      fromChar1 = fromChar2;
      fromChar2 = v;
    ...
    if (fromLine2 <
        fromLine1 || (fromLine1 == fromLine2 && fromWord2 < fromWord1)) {
      v = fromWord1;
      fromWord1 = fromWord2;
      fromWord2 = v;
    }
  ...
  bool doLinesIntersectY(TextLine line1, TextLine line2) {
    ...
    if (totalWidth > (line1.bounds.left - line2.bounds.right).abs() &&
        totalWidth > (line2.bounds.left - line1.bounds.right).abs()) {
      return true;
    }
  ...
  void calculateRects() {
    ...
    if (fromLine2 - fromLine1 + 1 >= 2) {
      left = isCharFull1 ? textLines[fromLine1].wordCollection[fromWord1]
          .glyphs[fromChar1].bounds.left : textLines[fromLine1]
          .wordCollection[fromWord1].glyphs[fromChar1].bounds.right;
      ...
      right = isCharFull2 ? textLines[fromLine2].wordCollection[fromWord2]
          .glyphs[fromChar2].bounds.right : textLines[fromLine2]
          .wordCollection[fromWord2].glyphs[fromChar2].bounds.left;
  ...
  bool doLinesIntersectX(TextLine line1, TextLine line2) {
    ...
    if (totalHeight > (line1.bounds.top - line2.bounds.bottom).abs() &&
        totalHeight > (line2.bounds.bottom - line1.bounds.top)) {
      return true;
    }
  ...
  double getDist(double x, double y, TextLine line) {
    return (line.bounds.centerLeft.dy - y).abs() +
        (line.bounds.topCenter.dx - x).abs();
  }
  ...
  String getText() {
    ...
    List<TextGlyph> glyphs1 = textlines[fromLine1].wordCollection[fromWord1]
        .glyphs;
    List<TextGlyph> glyphs2 = textlines[fromLine2].wordCollection[fromWord2]
        .glyphs;
  ...
  void dispose() {
    currentPage = page;
    StorageRepository.putInt('currPage', page);
    super.dispose();
  }
}
